...
def benchmark_infer(args, tokenizer, data, client=None, seed=42): #1234
    """
...
def benchmark_preparation(data_obj, partition, args, seed=1234):
    ...
    if args.shots > 0:
        if not args.cot and data_obj.train_data is None:
            print('Load train data for few shot learning')
            data_obj.load_data(partition='train', local_path=args.local_path)
            data_obj.preprocessing(partition='train')

        print(f'FEW SHOTS: {args.shots}')
...
def main(args):
    ...
    print("Inference will be ran on partition:", partition)
    ...
    if args.shots > 0 and args.multi_seed:
        ...
        for seed in [1234, 432, 32]:
            ...
            print(f'Running inference on {args.benchmark} for {len(data_obj.test_data)} samples')
            seed_predictions = benchmark_infer(
                args, tokenizer,
                data_obj.test_data,
                client, seed=seed)
            predictions = pd.concat([predictions, seed_predictions])
            print(f'Finished seed {seed} - {len(predictions)} generations collected.')
    elif args.sc_cot:
        predictions = pd.DataFrame()
        for i in range(args.sc_branch):
            print(f'Start branch {i+1}/{args.sc_branch}')
            branches = benchmark_infer(args, tokenizer, data_obj.test_data, client)
            predictions = pd.concat([predictions, branches])
            print(f'Finished branch {i+1}/{args.sc_branch}, {len(predictions)} generations collected.')
    else:
    ...
    print(f'{len(predictions)} generations store for checkpoint: {args.checkpoint_name}.')
