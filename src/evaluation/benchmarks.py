...
RESULT_DIR = os.getenv('UTILITY_RESULT_DIR', "")
RESULT_DIR = RESULT_DIR or f"/assets/results/utility-benchmark/"
...
def load_instruction(prompt_name):
    ...
    """
    path = os.path.join(SCRIPT_DIR, 'evaluation', 'instructions.json')
...
class Benchmark:
        ...
        try:
            if self.subsets is None:
                load_dataset(self.hub_name,
                             cache_dir=os.path.join(RESULT_DIR, 'datasets'),
    ...
    def load_data(self, partition='train', local_path=None):
        ...
        :param local_path: str (optional), the path to a directory holding train and test json local data files.
        """
        print(f'\nLoading data for benchmark {self.name} with partition {partition} in {self.splits}.\n')
    ...
    def preprocessing(self, partition='train'):
        ...
        try:
            if partition == 'train':
                self.train_data = self.train_data.map(self.custom_preprocessing, load_from_cache_file=True, num_proc=64)
            elif partition in ['test', 'validation']:
                self.test_data = self.test_data.map(self.custom_preprocessing, load_from_cache_file=True, num_proc=64)
            else:
    ...
    def add_few_shot(self, shots=8, seed=42, load_cot=False):
        ...
        if load_cot:
            ...
            cot_path = os.path.join(SCRIPT_DIR, 'evaluation', 'prompt_cot', f"{COT_PROMPTS[self.name]}.jsonl")
            ...
            # print("***** seed", seed, demonstrations[2]['prompt'])
            few_shot_prompt = '\n\n'.join([
        ...
        print("Applying in context learning")
        self.test_data = self.test_data.map( _add_few_shot, load_from_cache_file=True, num_proc=64)
    ...
    def save_generations(self, checkpoint_name, shots=0):
        ...
        """
        path = os.path.join(RESULT_DIR, 'generations', self.name)
        if not os.path.exists(os.path.dirname(path)):
            os.makedirs(os.path.dirname(path))
        if shots == 0:
            file_name = os.path.join(path, f"{self.name}-{checkpoint_name}")
        else:
            file_name = os.path.join(path, f"{self.name}-{checkpoint_name}-{str(shots)}-shot")
        gen_path = file_name + ".jsonl"
        if os.path.isfile(gen_path):
            gen_path = file_name + f"-{datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}.jsonl"
    ...
    def load_generations(self, checkpoint_name):
        ...
        """
        path = os.path.join(RESULT_DIR, 'generations', f"{self.name}_{checkpoint_name}.json")
        if not os.path.exists(path):
...
class MedMCQA(Benchmark):
    ...
    def __init__(self, name='medmcqa') -> None:
        ...
        self.path = os.path.join(RESULT_DIR, 'datasets', self.dir_name)
...
class PubMedQAValidation(Benchmark):
    ...
    def __init__(self, name='pubmedqa') -> None:
        ...
        self.num_options = 3
        self.local_path = os.path.join(RESULT_DIR, 'datasets', 'pubmedqa_pubmedqa_validation.jsonl')
