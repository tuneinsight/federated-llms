...
def eval(output_full, answer, shot=False, cot=False, answer_type="mcq"):
    ...
    if "is:" in output:
        try:
            output = output.split("is:")[1].strip().lower()
        except Exception:
            return default
    if "\n##" in output:
        try:
...
def display(metric_dict, run_name, benchmark, subset=None, verbose=False):
    ...
    if subset is not None:
        print(f'{benchmark} / {subset} - {run_name}:')
    else:
        print(f'{metric_dict["accuracy"]:.3f}', end='\t')
    # print(f'\n\taccuracy: {metric_dict["accuracy"]:.3f}\n')
...
def main(args):
    ...
    answer_type = benchmark_output_type[args.benchmark]
    cot = args.cot
